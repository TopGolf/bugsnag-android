 ...
 */
internal class DeviceIdStore(val file: File, val logger: Logger) {
    ...
    internal fun loadDeviceId(logger: Logger, uuidProvider: () -> UUID): String? {
        return try {
            ...
            val deviceId = loadDeviceIdInternal(logger)

            if (deviceId?.id != null) {
                deviceId.id
            } else {
                return persistNewDeviceUuid(logger, uuidProvider)
            }
        } catch (exc: Throwable) {
     ...
     */
    private fun loadDeviceIdInternal(logger: Logger): DeviceId? {
        if (file.length() > 0) {
            try {
     ...
     */
    private fun persistNewDeviceUuid(
        logger: Logger,
        uuidProvider: () -> UUID
    ): String? {
        return try {
            ...
            file.outputStream().channel.use { channel ->
                persistNewDeviceIdWithLock(channel, logger, uuidProvider)
            }
        } catch (exc: Throwable) {
